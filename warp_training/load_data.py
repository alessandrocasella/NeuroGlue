...
class SparseDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        rotZval = random.randint(0, 90)
        distZval = random.randint(200, 500)  # zoom
        ...
        M = np.linalg.multi_dot([K, R, T, Kinv])

        # Apply matrix transformation
        # cv2.warpPerspective(image, M, (w, h), warped, cv2.INTER_NEAREST, cv2.BORDER_CONSTANT, 0)
        cv2.warpPerspective(image, M, (w, h), warped)
